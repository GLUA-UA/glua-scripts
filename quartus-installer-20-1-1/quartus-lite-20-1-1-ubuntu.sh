...
HOME_DIR="/home/$USER"
INSTALL_PATH="/home/$USER/.intel_fpga_lite/20.1.1"
...
MODELSIM_ASE_ENABLE=true
MODELSIM_AE_ENABLE=false
...
build_disabled_components_flag() {
    ...
    [ $MODELSIM_ASE_ENABLE == false ] && DISABLED_COMPONENTS_FLAG="${DISABLED_COMPONENTS_FLAG}modelsim_ase,"
    [ $MODELSIM_AE_ENABLE == false ] && DISABLED_COMPONENTS_FLAG="${DISABLED_COMPONENTS_FLAG}modelsim_ae,"
...
agreements() {
    ...
    Intel does not warrant or assume responsibility for the accuracy or completeness of any information, text, graphics, links or other items within the Software.
    " > $t; whiptail --title "Legal Disclaimer" --scrolltext --textbox $t 20 78; rm $t
    if (! whiptail --title "Legal Disclaimer" --yesno --yes-button Accept --no-button Reject "           Do you accept the terms of the legal disclaimer?" 7 75) then
        whiptail --title "Quartus Prime Lite Installer" --msgbox "                 Installation aborted by user request." 7 75
        exit 1
    fi
...
options() {
    ...
    "modelsim_ase" "ModelSim-ASE (Free)                  (Recommended)  " ON 3>&1 1>&2 2>&3)
    ...
    if [ $exit_status -eq 0 ]; then
        ...
        for choice in "${CHOICES[@]}"; do
            ...
            case $choice in
                ...
                "modelsim_ase") MODELSIM_ASE_ENABLE=true ;;
            esac
        done
...
mkdir -p /tmp/quartus-prime-lite-installer-20-1-1/
wget -O /tmp/quartus-prime-lite-installer-20-1-1/quartus-lite.tar https://downloads.intel.com/akdlm/software/acdsinst/20.1std.1/720/ib_tar/Quartus-lite-20.1.1.720-linux.tar
tar -xvf /tmp/quartus-prime-lite-installer-20-1-1/quartus-lite.tar -C /tmp/quartus-prime-lite-installer-20-1-1/
...
rm -rf /tmp/quartus-lite-installer-20-1-1/
...
if [ "$QUARTUS_ENABLE" == "true" ]; then
    ...
    echo -e "[Desktop Entry]\nName=Quartus Prime Lite 20.1.1\nType=Application\nTerminal=false\nExec=$INSTALL_PATH/quartus/bin/quartus --64bit\nIcon=$INSTALL_PATH/quartus/adm/quartusii.png\nCategories=Development;Electronics;\nHidden=false\nNoDisplay=false\nStartupNotify=false" | tee $HOME_DIR/.local/share/applications/quartus_prime_lite_20_1_1.desktop
fi
